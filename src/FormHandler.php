...
class FormHandler
{
	...
	public function __construct()
	{
        ...
        $from_email ='forms@'.$host;
   		$this->mailer->setFrom($from_email,'Enviado desde Sitio Web',false);

        //SMTP Settings
        $this->mailer->isSMTP();
        $this->mailer->Host = "smtp.gmail.com";
        $this->mailer->SMTPAuth = true;
        $this->mailer->Username = "capitalmarketingagency838@gmail.com"; //enter you email address
        $this->mailer->Password = 'KMeR@zsAp46W'; //enter you email password
        $this->mailer->Port = 465;
        $this->mailer->SMTPSecure = "ssl";
        //Email Settings
        $this->mailer->isHTML(true);
        $this->mailer->addAddress("heyner717@gmail.com"); //enter you email address
     ...
     * @return FormHandler form handler object itself so that the methods can be chained
     */
	...
	{
		$this->$validator_fn($this->validator);
		return $this;
	}
 ...
 */
	...
	private function compose_mail($post)
	{
		...
		{
			$content .= "<strong>".ucwords($name).': '."</strong>";
			$content .= $value."<br/>";
			// dd($content);
		}
		//$this->mailer->Body  = "<strong>{$content}</strong>";

		$email_template = file_get_contents('mailform.html');
		$email_template = str_replace('$content',$content,$email_template);

		$this->mailer->Body = $email_template;
